...
# 4. [[community_gene_list]]: list of genes in rewired communities (nneded for enrichment analysis)
# 5. Enrichment GO BP
# 1. Get index objects -------------------------------------------------------
...
#rewcom
#mycoms <- c(1,7,8,9,13,21,22,40,48) # THIS FOR THE TABLE
mycoms <- mycoms <- c(8,13,22,40)
...
# 4. get targets from communities --------------------------------------------

# Function to get targets of communities

get_community_targets <- function(Com,linkeroutput_modules ){
  # Com is data.frame summary of comm_summ object
  # linkeroutput_modules is the module section of GRN inferrence method output
  
  mod_all <- c()
  for (i in seq(1, dim(Com)[1])){
    mod <- Com$Module[i]
    boot <- Com$Bootstrap[i]
    mod_all <- c(mod_all,sum(nrmod[1:boot-1])+mod)
  }
  return(unique(unlist(sapply(linkeroutput_modules[mod_all], function(x) x$target_genes))))
}

community_gene_list <- lapply(comm_summ, function(x){
  Com <- x$data
  targs <- get_community_targets(Com, linkeroutput_modules = linkeroutput10)
  regs <- names(x$regs_u)
  return(c(regs,targs))
})

# 5. ENRICHMENT -----------------------------------------------------------
# Map SYMBOL ids with ENTREZ
library("org.Hs.eg.db")
OrgDb <- org.Hs.eg.db 
genelist <- lapply(community_gene_list, function(x){mapIds(OrgDb, column="ENTREZID",
                                                           keytype="SYMBOL",
                                                           multiVals="first",
                                                           keys=x)})
# Compare cluster
# BP
names(genelist) <- c("Module 8", "Module 13","Module 22","Module 40")
...
pdf("./figures_TraRe_Community_enrichment/goBP_communities_8_13_22_40.pdf", height=15, width=12)

clusterProfiler::dotplot(BP,showCategory=10,title = "GO Biological Proces enrichment analysis")+scale_color_viridis()
